...
// for world-bounds. A cell is walkable when its CENTRE (where the avatar's feet
// anchor) sits on clearly-opaque sand (alpha > 200), which keeps the avatar off
// the ocean and off the faint anti-aliased waterline without eroding inland —
// so the full coastal perimeter, incl. the strip around the treehouse, is
// walkable. This — not LAND_ROWS (sampled from the legacy, unrendered
// home-island.png) — is the avatar's walk base. Generated by
// tools/island-art/gen-walkgrid.mjs (AT=200 ERODE=0) — do not hand-edit.
// Reachability from spawn to every zone is verified there.
